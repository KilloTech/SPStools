#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Sen3A-olci-eurol.py YYYYmmdd
# Sentinel-3A OLCI L1b ERR — true_color + ocean_color | Area: eurol
# Channel: E2H-S3A-02 | Reader: olci_l1b
...
segdir     = '/home/sps/received/hvs-2/E2H-S3A-02'
toodir     = '/home/sps/SPStools'
outbase    = '/home/sps/SPSdata/products/Sen3A/OLCI'
tmpdir     = f'/home/sps/SPSdata/tmp/olci_S3A_{Dat}'
area       = 'eurol'
receiver   = 'TBS-6909X'
service    = 'HVS-2'
channel    = 'E2H-S3A-02'
...
tarfiles = sorted(glob.glob(f'{segdir}/S3A_OL_1_ERR____{Dat}*.SEN3.tar'))
if not tarfiles:
    shutil.rmtree(tmpdir, ignore_errors=True)
...
if not eu_tars:
    eu_tars = tarfiles[:1]
    print(f'[Sen3A-olci] WARNING: no orbit in EU window, using first available')

print(f'[Sen3A-olci] Found {len(eu_tars)} EU orbit(s) for {Dat}')

for tf in eu_tars[:1]:   # Process only the first EU orbit
    base    = os.path.basename(tf).replace('.SEN3.tar', '')
    sen3dir = f'{tmpdir}/{base}.SEN3'

    print(f'[Sen3A-olci] Extracting {os.path.basename(tf)} ...')
    try:
    ...
    print(f'[Sen3A-olci] Loading {len(files)} NC files ...')
    ...
    print(f'[Sen3A-olci] Resampling to {area} ...')
    try:
    ...
    for comp in composites:
        tmpimg  = f'{tmpdir}/S3A_olci_{comp}_{Dat}_{Hou}{Min}.png'
        outfile = f'{outbase}/S3A-{Dat}-{Hou}{Min}-{comp}-eurol.jpg'
        try:
            lscene.save_dataset(comp, filename=tmpimg, writer='simple_image')
            ann   = f'Sentinel-3A OLCI {comp}  {Yea}-{Mon}-{Day} {Hou}:{Min} UTC | {service}/{channel}'
            ...
            if os.path.exists(outfile):
                ...
                print(f'[Sen3A-olci] OK: {os.path.basename(outfile)} ({sz:.1f} MB)')
            else:
