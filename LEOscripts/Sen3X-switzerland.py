...
#************************************************************************
#
# Satellite: Sentinel-3X                                     Sensor: OLCI
#
...
# Metop GDS. This script combines Sentinel-3A and Sentinel-3B passes in
# stacked images. Use it for latitudes >+40° or <-40° and smaller areas.
#
...
# Example name : S3X_OL_1_ERR____20220520T003318_20220520T011743_20220520T031351_2665_085_259______MAR_O_NR_002.SEN3.tar
...
# EUMETCast    : Sentinel-3A files arrive as above in channel E2H-S3A-02 (HVS-2)
# EUMETCast    : Sentinel-3B files arrive as above in channel E2H-S3B-02 (HVS-2)
...
if OS == 'Linux':
    segdir = ['/srv/E2H-S3A-02', '/srv/E2H-S3B-02']
    isbulk = [False, False]
elif OS == 'Windows':
    segdir = ['Z:/E2H-S3A-02', 'Z:/E2H-S3B-02']
    isbulk = [False, False]
else:
...
Yea, Mon, Day, Hou, Min, height = leo_images(Yea, Mon, Day, sat, NoD, True, segdir, [False, False], isbulk, 'olci_l1b',
...
for composite in composites:
    ...
    magick = get_magick(Yea, Mon, Day, Hou, Min, height, logo1, logo2, service, channel, receiver,
                        sat, sensor, composite, area, testrun, NoD, True, ADDlegend,
