...
for area in ['dwda', 'dwdn', 'dwdc', 'dwdi']:  # dwda restored: available from E1B-DWDSAT

    if area in ['dwdc', 'dwdi'] and int(Hou)%12:
        continue
    if area == 'dwdn' and int(Hou)%6:
        continue
    if area == 'dwda' and int(Hou)%3:
        continue

    # You have to define a list of chart types (one type per composite) that must fit to your composites
    if area in ['dwdi', 'dwda']:  # dwda from DWDSAT is also B&W
        Types = ['ww', 'ww', 'bb']
    else:
        Types = ['wc', 'wc', 'bc']
