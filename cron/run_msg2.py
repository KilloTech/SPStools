#!/usr/bin/env python3
# Wrapper cron per MSG2 IODC 3km - slot ogni 15 minuti UTC
# Cron: */15 * * * * (lookback 20 min per attesa file, come MSG3)
...
logging.info(f"MSG2 start slot={slot_str}")
r = subprocess.run(
    ["/home/sps/miniconda3/envs/pytroll/bin/python3",
     "/home/sps/SPStools/GEOscripts/MSG2-msg_ioc_3km.py",
     slot_str],
    capture_output=True, text=True
)
logging.info(f"MSG2 done slot={slot_str} rc={r.returncode}")
if r.returncode != 0:
    logging.error(f"MSG2 stderr: {r.stderr[-500:]}")
